...
*   **Domain Data Modelling [✓]**: Strict interfaces representing the API endpoints are declared in [`src/types.ts`](file:///d:/New%20folder%20(2)/FM%E1%BB%81m/L2/ajt-devdash-HoangDH50/src/types.ts).
...
*   **Error State UI [✓]**: Handled via `try/catch/finally` inside [`src/main.ts`](file:///d:/New%20folder%20(2)/FM%E1%BB%81m/L2/ajt-devdash-HoangDH50/src/main.ts) with full UX recovery.
...
*   **Generic `fetchJson<T>` Helper [✓]**: A reusable, type-safe API client helper in [`src/api.ts`](file:///d:/New%20folder%20(2)/FM%E1%BB%81m/L2/ajt-devdash-HoangDH50/src/api.ts) that checks `response.ok` status.
...
*   **Union/Literal App State [✓]**: Application state is modeled as a literal status union (`'loading' | 'error' | 'success' | 'detail'`) in [`src/types.ts`](file:///d:/New%20folder%20(2)/FM%E1%BB%81m/L2/ajt-devdash-HoangDH50/src/types.ts).

### 🟪 Excellent Tier (10.0 Points) — Completed
*   **Discriminated Union State [✓]**: Unified state is exhaustively narrowed in the UI router `renderApp()` inside [`src/ui.ts`](file:///d:/New%20folder%20(2)/FM%E1%BB%81m/L2/ajt-devdash-HoangDH50/src/ui.ts) using `assertNever(state)`.
...
*   **Generic Class with Constraints [✓]**: Designed a custom `CacheManager<T extends Identifiable>` inside [`src/utils.ts`](file:///d:/New%20folder%20(2)/FM%E1%BB%81m/L2/ajt-devdash-HoangDH50/src/utils.ts) to cache details call responses.
...
1.  **Scope, Closures, and Debounce/Memoize**
    *   See [`src/utils.ts`](file:///d:/New%20folder%20(2)/FM%E1%BB%81m/L2/ajt-devdash-HoangDH50/src/utils.ts#L22) for the private variable closure inside `debounce`.
    *   See [`src/utils.ts`](file:///d:/New%20folder%20(2)/FM%E1%BB%81m/L2/ajt-devdash-HoangDH50/src/utils.ts#L43) for the Map closure inside `memoize`.
...
3.  **Parallel Loading with Promise.all**
    *   See [`src/main.ts`](file:///d:/New%20folder%20(2)/FM%E1%BB%81m/L2/ajt-devdash-HoangDH50/src/main.ts#L130) and [`src/main.ts`](file:///d:/New%20folder%20(2)/FM%E1%BB%81m/L2/ajt-devdash-HoangDH50/src/main.ts#L254) for concurrent calls.
    *   References: `02_Asynchronous_JavaScript.md#5-promise-combinators-promiseall-and-friends`
4.  **Generics & Constraints**
    *   See [`src/api.ts`](file:///d:/New%20folder%20(2)/FM%E1%BB%81m/L2/ajt-devdash-HoangDH50/src/api.ts#L23) for `fetchJson<T>`.
    *   See [`src/utils.ts`](file:///d:/New%20folder%20(2)/FM%E1%BB%81m/L2/ajt-devdash-HoangDH50/src/utils.ts#L61) for `CacheManager<T extends Identifiable>`.
    *   References: `03_TypeScript.md#9-generics`
5.  **Exhaustive Narrowing**
    *   See [`src/ui.ts`](file:///d:/New%20folder%20(2)/FM%E1%BB%81m/L2/ajt-devdash-HoangDH50/src/ui.ts#L268) for the compiler check on `AppState`.
    *   References: `03_TypeScript.md#6-type-narrowing`
6.  **Utility Types**
    *   See [`src/types.ts`](file:///d:/New%20folder%20(2)/FM%E1%BB%81m/L2/ajt-devdash-HoangDH50/src/types.ts#L88) for `Pick`, `Omit`, and `Partial`.
